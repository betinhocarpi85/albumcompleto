...
export async function POST(request: Request) {
  ...
  if (!['free', 'mensal', 'anual'].includes(plano)) {
  ...
  const { error } = await sb.from('profiles').update({ plano, plano_expira }).eq('id', userId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  await sb.from('admin_logs').insert({
    action: 'set_plan',
    target_id: userId,
    details: `Plano: ${plano}, meses: ${meses ?? 0}, expira: ${plano_expira ?? 'nunca'}`,
  })
